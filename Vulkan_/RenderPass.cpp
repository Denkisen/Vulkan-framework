...
namespace Vulkan
{
  ...
  RenderPass_impl::RenderPass_impl(const std::shared_ptr<Device> dev, const std::shared_ptr<SwapChain> swapchain, const RenderPassConfig &params) noexcept
  {
  ...
  VkResult RenderPass_impl::ReCreate() noexcept
  {
    vkDeviceWaitIdle(device->GetDevice());
    std::lock_guard lock(render_pass_mutex);
    return Create();
  }

  VkResult RenderPass_impl::Create() noexcept
  {
    Clear();
  ...
  RenderPass &RenderPass::operator=(RenderPass &&obj) noexcept
  {
    ...
    {
      std::scoped_lock lock(obj.impl->render_pass_mutex, impl->render_pass_mutex);
      impl = std::move(obj.impl);
    }
    else if (obj.impl.get())
    {
      std::lock_guard lock(obj.impl->render_pass_mutex);
      impl = std::move(obj.impl);
    }
    else
    {
